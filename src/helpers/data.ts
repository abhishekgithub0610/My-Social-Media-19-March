...
export const getAllUserConnections = async (): Promise<ConnectionsType[]> => {
  const data = userConnections.map((connection) => {
    ...
    return {
      ...connection,
      user,
      // ✅ ADD THIS: Precompute random value outside render
      sharedConnectionCount: Math.floor(Math.random() * 10),
    };
  });
  await sleep();
  return data;
...
export const getUserForAllComments = (
...
export const getAllFeeds = async (): Promise<SocialPostType[]> => {
  const data = socialPostsData.map((post) => {
    ...
    const commentsData = socialCommentsData.filter(
