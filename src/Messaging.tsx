...
import { useFetchData } from "@/useFetchData"; // to be confirmed/deleted
      ...
      <div className="flex-grow-1">
        <div className="w-100">
          <div
            className={clsx(
              "d-flex flex-column",
              received ? "align-items-end" : "align-items-start",
            )}
          >
            {message.image ? (
...
const UserCard = ({
  ...
  return (
    <>
      ...
      >
        <div
          className={clsx(`avatar status-${user.status}`, {
        ...
        <div className="small ms-auto text-nowrap">
          {" "}
          {timeSince(user.lastActivity)}{" "}
        </div>
      </li>
    </>
  );
...
const Messaging = () => {
  ...
  const { isTrue: isOpenCollapseToast, toggle: toggleToastCollapse } =
    useToggle(true);
  ...
  return (
    <>
      <ul className="list-unstyled">
        {allUserMessages?.map((user, idx) => (
          <UserCard user={user} key={idx} openToast={setTrue} />
        ))}
            ...
            <ToastHeader closeButton={false} className="bg-mode">
              <div className="d-flex justify-content-between align-items-center w-100">
                <div className="d-flex">
                  <div
                    className={clsx("flex-shrink-0 avatar me-2", {
